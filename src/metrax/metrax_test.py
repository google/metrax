...
IOU_NUM_CLASSES = 3
IOU_TARGETS = np.random.randint(0, IOU_NUM_CLASSES, size=(2, 8, 8)).astype(np.int32)
IOU_PREDICTIONS = np.random.randint(0, IOU_NUM_CLASSES, size=(2, 8, 8)).astype(np.int32)
...
class MetraxTest(parameterized.TestCase):
  ...
  def test_metrics_jittable(self, metric, kwargs):
    ...
    np.testing.assert_allclose(computed_metric.compute(), jitted_metric.compute(), rtol=1e-2, atol=1e-2)
  ...
  def test_metrics_not_jittable(self, metric, kwargs):
    ...
    np.testing.assert_raises(TypeError, lambda: jax.jit(metric.from_model_output)(**kwargs))
