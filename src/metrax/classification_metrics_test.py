...
class ClassificationMetricsTest(parameterized.TestCase):
  ...
  def test_binary_accuracy_broadcasting(self):
    """Test that `BinaryAccuracy` correctly squeezes mismatching trailing 1s."""
    # predictions: (B, 1), labels: (B,), sample_weights: (B,)
    preds2d = jnp.array([[0.1], [0.8], [0.9], [0.2]])
    labels1d = jnp.array([0.0, 1.0, 0.0, 0.0])
    weights1d = jnp.array([1.0, 2.0, 1.0, 1.0])

    m = metrax.BinaryAccuracy.from_model_output(
        predictions=preds2d,
        labels=labels1d,
        sample_weights=weights1d,
    )
    np.testing.assert_allclose(m.compute(), 0.8)

    # predictions: (B,), labels: (B, 1), sample_weights: (B, 1)
    preds1d = jnp.array([0.1, 0.8, 0.9, 0.2])
    labels2d = jnp.array([[0.0], [1.0], [0.0], [0.0]])
    weights2d = jnp.array([[1.0], [2.0], [1.0], [1.0]])

    m2 = metrax.BinaryAccuracy.from_model_output(
        predictions=preds1d,
        labels=labels2d,
        sample_weights=weights2d,
    )
    np.testing.assert_allclose(m2.compute(), 0.8)

    # predictions: (B, 1), labels: (B, 1), sample_weights: (B, 1)
    preds2d_all = jnp.array([[0.1], [0.8], [0.9], [0.2]])
    labels2d_all = jnp.array([[0.0], [1.0], [0.0], [0.0]])
    weights2d_all = jnp.array([[1.0], [2.0], [1.0], [1.0]])

    m3 = metrax.BinaryAccuracy.from_model_output(
        predictions=preds2d_all,
        labels=labels2d_all,
        sample_weights=weights2d_all,
    )
    np.testing.assert_allclose(m3.compute(), 0.8)
