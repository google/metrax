...
AUDIO_SHAPE_1D = (1000,)
AUDIO_TARGET_1D = np.sin(np.linspace(0, 2 * np.pi * 5, AUDIO_SHAPE_1D[0])).astype(np.float32)
AUDIO_PREDS_1D_NOISY = (AUDIO_TARGET_1D + 0.1 * np.random.randn(*AUDIO_SHAPE_1D)).astype(np.float32)
...
AUDIO_PREDS_2D_NOISY = (AUDIO_TARGET_2D + 0.5 * np.random.randn(*AUDIO_SHAPE_2D)).astype(np.float32)
